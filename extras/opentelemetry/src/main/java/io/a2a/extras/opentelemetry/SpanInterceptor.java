...
import static io.a2a.server.interceptors.A2AObservabilityNames.ERROR_TYPE;
import static io.a2a.server.interceptors.A2AObservabilityNames.GENAI_RESPONSE;
...
public class SpanInterceptor {
    ...
    public Object trace(jakarta.interceptor.InvocationContext jakartaContext) throws Exception {
        ...
        Kind kind = jakartaContext.getMethod().getAnnotation(Trace.class).kind();
        Class<? extends AttributeExtractor> extractorClass = jakartaContext.getMethod()
        ...
        SpanKind spanKind = SpanKind.valueOf(kind.toString());
        ...
        if (extractorClass != null && !extractorClass.equals(NoAttributeExtractor.class)) {
            try {
                extractor = extractorClass.getDeclaredConstructor().newInstance();
                Map<String, String> attributes = extractor.get().apply(customContext);
                for (Map.Entry<String, String> attribute : attributes.entrySet()) {
                    spanBuilder.setAttribute(attribute.getKey(), attribute.getValue());
                }
            } catch (Exception e) {
                LOGGER.warn("Failed to instantiate attribute extractor {}: {}", extractorClass.getName(), e.getMessage(), e);
            }
        }
        ...
        try (Scope scope = span.makeCurrent()) {
            ...
            if (ret != null) {
                if (ret instanceof Flow.Publisher publisher) {
                    return AsyncUtils.convertingProcessor(publisher, event -> transformEvent(event, name, spanKind, span.getSpanContext()));
                } else {
                    if(extractor != null && extractor.extractResponse()) {
                        span.setAttribute(GENAI_RESPONSE, ret.toString());
                    }
                }
            }
            return ret;
        } catch (Exception ex) {
            span.setAttribute(ERROR_TYPE, ex.getMessage());
    ...
    private Object transformEvent(Object event, String name, SpanKind kind, SpanContext spanContext) {
        SpanBuilder spanBuilder = tracer.spanBuilder(name + "-event")
                .addLink(spanContext)
                .setSpanKind(kind);
        if (event != null) {
            Span span = spanBuilder.startSpan();
            try (Scope scope = span.makeCurrent()) {
                span.setAttribute("gen_ai.agent.a2a.response.event", event.toString());
            } finally {
                span.end();
            }
        }
