 ...
 */
public class GrpcAttributeExtractor implements AttributeExtractor {
    ...
    private @NonNull Map<String, String> extractAttributes(Object @NonNull [] parameters) {
        ...
        if (parameters.length > 0 && parameters[0] != null && extractRequest()) {
        ...
        if (extensions != null) {
            attributes.put(GENAI_EXTENSIONS, extensions);
        }
        attributes.put(GENAI_PROTOCOL, "GRPC");
