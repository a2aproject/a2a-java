...
public class A2AServerRoutes {
    ...
    public void resubscribeTask(RoutingContext rc) {
        ...
        try {
            if (taskId == null || taskId.isEmpty()) {
                ...
                HTTPRestResponse response = jsonRestHandler.resubscribeTask(taskId, extractTenant(rc), context);
                if (response instanceof HTTPRestStreamingResponse hTTPRestStreamingResponse) {
                    streamingResponse = hTTPRestStreamingResponse;
                } else {
                    error = response;
    ...
    @Route(regex = "^\\/(?<tenant>[^\\/]*\\/?)tasks\\/(?<id>[^/]+)\\/pushNotificationConfigs$", order = 1, methods = {Route.HttpMethod.POST}, consumes = {APPLICATION_JSON}, type = Route.HandlerType.BLOCKING)
    ...
    @Route(regex = "^\\/(?<tenant>[^\\/]*\\/?)tasks\\/(?<id>[^/]+)\\/pushNotificationConfigs\\/(?<configId>[^/]+)", order = 1, methods = {Route.HttpMethod.DELETE}, type = Route.HandlerType.BLOCKING)
    ...
    private String extractTenant(RoutingContext rc) {
        ...
        if (tenant.endsWith("/")) {
            tenant = tenant.substring(tenant.lastIndexOf('/'));
        }
        return tenant;
    }
    /**
    ...
    @Route(regex = "^\\/(?<tenant>[^\\/]*\\/?)tasks\\/card$", order = 1, methods = Route.HttpMethod.GET, produces = APPLICATION_JSON)
