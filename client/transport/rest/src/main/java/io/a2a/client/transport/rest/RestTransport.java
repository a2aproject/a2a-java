...
public class RestTransport implements ClientTransport {
    ...
    public Task getTask(TaskQueryParams taskQueryParams, @Nullable ClientCallContext context) throws A2AClientException {
        ...
        try {
            ...
            if (taskQueryParams.historyLength() != null && taskQueryParams.historyLength() > 0) {
                url = buildBaseUrl(taskQueryParams.tenant()) + String.format("/tasks/%1s?historyLength=%2d", taskQueryParams.id(), taskQueryParams.historyLength());
            } else {
                url = buildBaseUrl(taskQueryParams.tenant()) + String.format("/tasks/%1s", taskQueryParams.id());
            }
    ...
    private String extractTenant(@Nullable String tenant) {
        ...
        if(!tenantPath.endsWith("/")){
            tenantPath = tenantPath.substring(tenantPath.lastIndexOf('/'));
        }
        return tenantPath;
    }
