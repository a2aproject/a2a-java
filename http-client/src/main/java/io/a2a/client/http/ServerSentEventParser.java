    ...
    private void processField(String field, String value) {
        switch (field) {
            case "data" -> {
                ...
                // Check total byte size to prevent OOM on large streams
                if (dataBufferBytes + value.length() > MAX_BUFFER_BYTES) {
                    handleError(new IllegalStateException("SSE data buffer exceeded maximum byte size of " + MAX_BUFFER_BYTES + " bytes"));
                    skippingCurrentEvent = true;
                    dataBuffer.clear();
                    dataBufferBytes = 0;
                    return;
